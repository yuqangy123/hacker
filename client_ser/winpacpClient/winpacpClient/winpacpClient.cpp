// winpacpClient.cpp : 定义控制台应用程序的入口点。
//
...
struct IP_HEADER
{
	...
	byte totalLen[2];
	byte seqNumber[2];
	byte flagAndFragPart[2];//（如SYN,ACK,等等)
	byte ttl;
	byte hiProtovolType;
...
int EncodeSynPacket(byte packet[], const char *lpszSrcIpAddr, byte srcMacAddr[],
	const char *lpszDstIpAddr, unsigned int dstPort)
{
	TCP_HEADER tcpHeader;
	memset(&tcpHeader, 0, sizeof tcpHeader);
	*(unsigned short *)tcpHeader.srcPort = htons(8847);
	*(unsigned short *)tcpHeader.dstPort = htons(dstPort);
	*(unsigned int *)tcpHeader.seqNumber = htonl(0xAFFF);
	*(unsigned int *)tcpHeader.ackNumber = htonl(0x00);
	tcpHeader.headLen = (sizeof(TCP_HEADER) / 4 << 4 | 0);//5 << 4;
	tcpHeader.contrl = 1 << 1;
	*(unsigned short *)tcpHeader.wndSize = htons(16384);

	IP_HEADER ipHeader;
	memset(&ipHeader, 0, sizeof ipHeader);

	ipHeader.versionAndHeader = (4<<4 | sizeof(ipHeader)/sizeof(unsigned long));//版本 + 头长度
	*(unsigned short *)ipHeader.totalLen = htons(sizeof(IP_HEADER) + sizeof(TCP_HEADER));
	*(unsigned short *)ipHeader.seqNumber = 2;
	*(unsigned short *)ipHeader.flagAndFragPart = 0;

	ipHeader.ttl = 128;
	ipHeader.hiProtovolType = IPPROTO_TCP;
	memset(&ipHeader.headerCheckSum, 0, sizeof(ipHeader.headerCheckSum));
...
//https://blog.csdn.net/sunny1996/article/details/61636124
int main()
{
	...
	ipList.push_back("192.168.0.105");

	byte packet[1024];
	int size = EncodeSynPacket(packet, "192.168.0.101", localMacAddr, ipList[0].c_str(), 123);
	...
	unsigned int baseIpAddr = ntohl(inet_addr("192.168.0.101"));
	...
	*(unsigned short *)(psdHeader->tcpLen) = htons(sizeof(TCP_HEADER));
	psdHeader->protocol = 6;//IPPROTO_TCP
	...
	while (1)
	{
		...
		*(unsigned short *)(pTcpHeader->checkSum) = CheckSum((unsigned short *)psdPacket, sizeof(PSDTCP_HEADER) + sizeof(TCP_HEADER));
		...
		//system("pause");
		Sleep(0);
		int ret = pcap_sendpacket(handle, packet, size);
		int test = 0;
	}
